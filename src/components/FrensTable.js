...
export default function FrensTable({ searchInput, page }) {
  const {
    data: frensData,
    error,
    isValidating,
  ...
  return (
    <AnimatePresence exitBeforeEnter>
      ...
      >
        <FrensTablePage frens={frens} />
      </motion.div>
    </AnimatePresence>
  );
}

const FrensTablePage = ({ frens }) => (
  <div
                ...
                {fren.ens === ".eth" ? (
                  ...
                  >
                    <Image
                      ...
                      priority={inx < 10}
                    />
                    <span className={frensTableStyles.ensName}>{fren.ens}</span>
                    <svg
                      width="16"
                      height="15"
                      viewBox="0 0 16 15"
